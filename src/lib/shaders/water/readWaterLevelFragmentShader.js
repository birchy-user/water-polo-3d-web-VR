...
export const readWaterLevelFragmentShader = /* glsl */`
    ...
    void main()	{
        ...
        vec2 normal = vec2(
            ( texture2D( levelTexture, point1 + vec2( - cellSize.x, 0 ) ).x - texture2D( levelTexture, point1 + vec2( cellSize.x, 0 ) ).x ) * TEXTURE_WIDTH / WATER_SURFACE_SIZE,
            ( texture2D( levelTexture, point1 + vec2( 0, - cellSize.y ) ).x - texture2D( levelTexture, point1 + vec2( 0, cellSize.y ) ).x ) * TEXTURE_WIDTH / WATER_SURFACE_SIZE );
